...
function doWork() {
    ...
    local add_arr
    case "${OS_ID}" in
        arch) add_arr=(polkit python python-pip terminus-font xorg-server xorg-xrdb xorg-xinit unzip ttf-dejavu upower acpilight picom noto-fonts ilock xorg-xrandr autorandr hsetroot) ;;
        ubuntu)
